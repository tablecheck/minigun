...
module Minigun
  ...
  class InputQueue
    def initialize(queue, stage, expected_sources, stage_stats: nil)
      @queue = queue
      @stage = stage  # Stage object (not name)
    ...
    def pop
      loop do
        ...
        # Handle EndOfSource signals (source is now Stage object)
        if item.is_a?(EndOfSource)
          @sources_expected << item.source # Discover dynamic sources (objects)
  ...
  class OutputQueue
    ...
    # target can be Stage object or name (will be resolved)
    def to(target)
      ...
      # Track this as a runtime edge for END signal handling (use Stage objects)
